...
KairosDB.prototype.version = function (callback) {
  this.execute('version', {}, null, function (err, result) {
    if (err)
      return callback(err);
    try {
...
KairosDB.prototype.datapoints = function (datapoints, callback) {
  this.execute('datapoints', {method: 'POST'}, datapoints, function (err) {
    if (err)
      return callback(err);
    try {
      ...
      return callback(new Error('Failed to save data points, ' + ex));
    }
  });
};
